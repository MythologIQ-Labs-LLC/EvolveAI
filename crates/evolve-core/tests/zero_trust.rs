...
async fn test_unverified_content_requires_more_evidence() {
    ...
    let unverified = proc
        .encode(&input("LLM says X", TrustLevel::Unverified), 1000)
        .await
        .unwrap();
    let verified = proc
        .encode(&input("Proven fact Y", TrustLevel::Verified), 1001)
        .await
        .unwrap();
    ...
    assert!(
        s2 > s1,
        "Verified source (σ₀=0.3) should saturate faster than Unverified (σ₀=0.0)"
    );
}
...
async fn test_crystallization_guard_prevents_hallucination_permanence() {
    ...
    let result = proc
        .encode(&input("hallucinated fact", TrustLevel::Unverified), 1000)
        .await
        .unwrap();
...
async fn test_disputed_memory_evaporates() {
    ...
    let result = proc
        .encode(&input("wrong claim", TrustLevel::Unverified), 1000)
        .await
        .unwrap();
    let addr = result.unit.address.clone();
    ...
    assert!(
        evolve_core::memory::decay::should_prune(weight, 0.05),
        "disputed memory should be prunable"
    );
}
...
async fn test_forget_removes_crystallized_memory() {
    ...
    assert_eq!(
        proc.stats().l3_size,
        0,
        "forget should remove even L3 memories"
    );
}
